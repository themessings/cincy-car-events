...
def is_automotive_event_safe(title: str, location: str, cfg: dict) -> bool:
    """Compatibility shim for older/newer is_automotive_event signatures."""
    fn = is_automotive_event

    # Preferred modern call
    try:
        return bool(fn(title, location, cfg=cfg))
    except TypeError:
        pass

    # Older positional signatures
    try:
        return bool(fn(title, location, cfg))
    except TypeError:
        pass

    # Legacy 2-arg signature (title, cfg)
    try:
        return bool(fn(title, cfg))
    except TypeError:
        pass

    # Last resort: allow event through rather than crashing collector
    log("⚠️ is_automotive_event signature mismatch; bypassing automotive filter for this item.")
    return True
