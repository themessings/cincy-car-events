...
def _resolve_verify_usps_address():
    """Return active USPS verifier function, or a safe no-op fallback."""
    verifier = globals().get("verify_usps_address")
    if callable(verifier):
        return verifier

    def _fallback(_location: str, _city_state: str) -> Dict[str, str]:
        return {
            "status": "unverified_runtime_missing_verifier",
            "street": "",
            "city": "",
            "state": "",
            "zip5": "",
            "zip4": "",
            "formatted": "",
            "error": "verify_usps_address_not_loaded",
        }

    return _fallback


def verify_usps_address_safe(location: str, city_state: str) -> Dict[str, str]:
    """Call USPS verifier if available; otherwise return non-fatal fallback."""
    verifier = globals().get("verify_usps_address")
    if callable(verifier):
        try:
            return verifier(location, city_state)
        except Exception as ex:
            return {
                "status": "unverified_runtime_verifier_error",
                "street": "",
                "city": "",
                "state": "",
                "zip5": "",
                "zip4": "",
                "formatted": "",
                "error": clean_ws(str(ex))[:200] or "verifier_runtime_error",
            }
    return _resolve_verify_usps_address()(location, city_state)
...
def update_apex_spreadsheet(events: List[dict]) -> None:
    ...
    for ev in events:
        ...
        if cache_key not in verification_cache:
            verification_cache[cache_key] = verify_usps_address_safe(location_text, city_state_text)
